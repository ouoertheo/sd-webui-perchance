...
# Usage
Download as a zip and extract contents to scripts folder. In Stable Diffusion WebUI, select the Perchance script, expand the Proxy section, start the server, put in the name of the desired generator then click Refresh. Put in `{perchance}` into the prompt, and it will be replaced with the generated perchance output.
